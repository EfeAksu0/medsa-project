...
exports.notifyNewTrade = exports.setupDiscordBot = void 0;
...
const setupDiscordBot = async () => {
    try {
        ...
        client_1.discordClient.on('messageCreate', async (message) => {
            ...
            if (message.content === '!setup') {
                ...
                try {
                    ...
                    const getOrCreateRole = async (name, color, hoist, position, permissions = []) => {
                    ...
                    const modRole = await getOrCreateRole('Moderator', '#EF4444', true, getPos(), [discord_js_1.PermissionFlagsBits.Administrator]);
                    ...
                    if (!rookieRole.hoist)
                        await rookieRole.edit({ hoist: true });
                    ...
                    const getOrCreateChannel = async (name, parentId, topic, type = 0) => {
                    ...
                    let genChannel = guild.channels.cache.find(c => c.name === 'general' && c.parentId === publicCat.id);
                    if (!genChannel) {
                    ...
                    // --- POPULATE CHANNELS ---
                    // 1. FAQ
                    if (faqChannel && faqChannel.type === discord_js_1.ChannelType.GuildText) {
                        const c = faqChannel;
                    ...
                    // 2. Tech Status
                    if (statusChannel && statusChannel.type === discord_js_1.ChannelType.GuildText) {
                        const c = statusChannel;
                        if ((await c.messages.fetch({ limit: 1 })).size === 0) {
                            await c.send({
                                embeds: [{
                                        title: '🔴 SYSTEM STATUS',
                    ...
                    // 4. Economic Calendar
                    if (calendarChannel && calendarChannel.type === discord_js_1.ChannelType.GuildText) {
                        const c = calendarChannel;
                        if ((await c.messages.fetch({ limit: 1 })).size === 0) {
                            const calButton = new discord_js_1.ButtonBuilder()
                    ...
                    if (announceChannel && announceChannel.type === discord_js_1.ChannelType.GuildText) {
                        const textAnnounce = announceChannel;
                    ...
                    // Post Rules
                    if (rulesChannel && rulesChannel.type === discord_js_1.ChannelType.GuildText) {
                        const textRules = rulesChannel;
                    ...
                    let battleCat = guild.channels.cache.find(c => c.name === '⚔️ BATTLEFIELD' && c.type === 4);
                    if (!battleCat) {
                    ...
                    // VIP Area
                    let vipCat = guild.channels.cache.find(c => c.name === '🔮 ORACLE (VIP)' && c.type === 4);
                    if (!vipCat) {
        ...
        await client_1.discordClient.login(token);
    }
    catch (error) {
        console.error('❌ Failed to start Discord Bot:', error);
    }
};
exports.setupDiscordBot = setupDiscordBot;
const notifyNewTrade = async (trade, userName) => {
    try {
        if (Number(trade.pnl) === 0 && trade.result === 'OPEN')
            return;
        const pnl = Number(trade.pnl || 0);
        const channelName = pnl >= 0 ? 'wins-and-profits' : 'loss-porn';
        const guild = client_1.discordClient.guilds.cache.first();
        if (!guild)
            return;
...
exports.notifyNewTrade = notifyNewTrade;
